...
private RButton businessDataIndexRecreationButton = null;  //  @jve:decl-index=0:visual-constraint="272,361"
private ULCPollingTimer recreateIndexTimer;  //  @jve:decl-index=0:
private IActionListener recreateIndexTimerListener;  //  @jve:decl-index=0:
  ...
  private RFlowLayoutPane getButtonPane() {
    if (buttonPane == null) {
      ...
      buttonPane.add(getBusinessDataIndexRecreationButton());
  ...
  /**
   * This method initializes businessDataIndexRecreationButton	
   * 	
   * @return ch.ivyteam.ivy.richdialog.widgets.components.RButton	
   */
  private RButton getBusinessDataIndexRecreationButton() {
  	if (businessDataIndexRecreationButton == null) {
  		businessDataIndexRecreationButton = new RButton();
  		businessDataIndexRecreationButton.setText("<%=ivy.cms.co(\"/ch/ivyteam/ivy/admin/tool/button/businessDataIndexRecreation\")%>");
  		businessDataIndexRecreationButton.setToolTipText("<%=ivy.cms.co(\"/ch/ivyteam/ivy/admin/tool/button/businessDataIndexRecreationTooltip\")%>");
  		businessDataIndexRecreationButton.setName("businessDataIndexRecreationButton");
  	}
  	return businessDataIndexRecreationButton;
  }
...
private RCheckBox getRestartRequired() {
  ...
  private void createUlcPollingTimer() {
    recreateIndexTimer = new ULCPollingTimer(1, null);
    recreateIndexTimer.setRepeats(true);
    recreateIndexTimerListener = new IActionListener() {
      private static final long serialVersionUID = 6857503452607466633L;
  ...
  public void stopPollingTimer() {
    if (recreateIndexTimer != null) {
      recreateIndexTimer.stop();
      recreateIndexTimer.removeActionListener(recreateIndexTimerListener);
    }
  }
  ...
  public void startPollingTimer() {
    ...
    recreateIndexTimer.addActionListener(recreateIndexTimerListener);
    recreateIndexTimer.start();
  }
