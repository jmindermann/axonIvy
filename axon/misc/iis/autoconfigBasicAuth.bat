...
SET VDIR=ivy
SET CFG_DIR=%~dp0
SET LOG_DIR=%CFG_DIR%basicAuthConfigLog
...
ECHO This Script is only tested with Microsoft Windows Server 2012.
...
ECHO This will configure Basic Authentication for an IIS.
...
ECHO == Install BasicAuthentication == >> %LOG_FILE%
ECHO Consult %IIS_INST_LOG% for more information about the installation process >> %LOG_FILE%
PKGMGR.EXE /l:%IIS_INST_LOG% /iu:IIS-BasicAuthentication >> %LOG_FILE%
ECHO .

ECHO. >> %LOG_FILE%
ECHO. >> %LOG_FILE%
ECHO == Enable Basic Authentication for virtual directory ivy == >> %LOG_FILE%
appcmd.exe set config "%SITE%/%VDIR%" -section:system.webServer/security/authentication/basicAuthentication /enabled:"True" /commit:apphost >> %LOG_FILE%
ECHO .
